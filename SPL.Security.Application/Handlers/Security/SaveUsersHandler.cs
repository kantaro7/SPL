namespace SPL.Security.Application.Handlers.Security
{
    ...
    public class SaveUsersHandler : IRequestHandler<SaveUsersCommand, ApiResponse<long>>
    {

        private readonly ISecurityInfrastructure _infrastructure;

        public SaveUsersHandler(ISecurityInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveUsersCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveUsers(request.Data);
