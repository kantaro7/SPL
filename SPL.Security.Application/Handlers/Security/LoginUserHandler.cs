namespace SPL.Security.Application.Handlers.Security
{
    ...
    public class LoginUserHandler : IRequestHandler<LoginUserCommand, ApiResponse<long>>
    {

        private readonly ISecurityInfrastructure _infrastructure;

        public LoginUserHandler(ISecurityInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(LoginUserCommand request, CancellationToken cancellationToken)
        {
            try
            {
