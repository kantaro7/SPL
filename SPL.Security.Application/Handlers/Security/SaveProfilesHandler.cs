namespace SPL.Security.Application.Handlers.Security
{
    ...
    public class SaveProfilesHandler : IRequestHandler<SaveProfilesCommand, ApiResponse<long>>
    {

        private readonly ISecurityInfrastructure _infrastructure;

        public SaveProfilesHandler(ISecurityInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveProfilesCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveProfiles(request.Data) ;
