namespace SPL.Security.Application.Handlers.Security
{
    ...
    public class DeleteProfilesHandler : IRequestHandler<DeleteProfilesCommand, ApiResponse<long>>
    {

        private readonly ISecurityInfrastructure _infrastructure;

        public DeleteProfilesHandler(ISecurityInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(DeleteProfilesCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.deleteProfiles(request.Data) ;
