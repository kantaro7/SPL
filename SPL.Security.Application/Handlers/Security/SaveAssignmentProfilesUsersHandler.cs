namespace SPL.Security.Application.Handlers.Security
{
    ...
    public class SaveAssignmentProfilesUsersHandler : IRequestHandler<SaveAssignmentProfilesUsersCommand, ApiResponse<long>>
    {

        private readonly ISecurityInfrastructure _infrastructure;

        public SaveAssignmentProfilesUsersHandler(ISecurityInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveAssignmentProfilesUsersCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveAssignmentProfilesUsers(request.Data);
