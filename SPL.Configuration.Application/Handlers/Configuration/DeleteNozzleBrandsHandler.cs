namespace SPL.Configuration.Application.Handlers.Configuration
{
    ...
    public class DeleteNozzleBrandsHandler : IRequestHandler<DeleteNozzleBrandsCommand, ApiResponse<long>>
    {

        private readonly IConfigurationInfrastructure _infrastructure;

        public DeleteNozzleBrandsHandler(IConfigurationInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(DeleteNozzleBrandsCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _infrastructure.deleteNozzleBrands(request.Data);
