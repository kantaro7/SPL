namespace SPL.Configuration.Application.Handlers.Configuration
{
    ...
    public class SaveNozzleBrandsHandler : IRequestHandler<SaveNozzleBrandsCommand, ApiResponse<long>>
    {

        private readonly IConfigurationInfrastructure _infrastructure;

        public SaveNozzleBrandsHandler(IConfigurationInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveNozzleBrandsCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _infrastructure.saveNozzleBrands(request.Data);
