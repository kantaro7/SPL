namespace SPL.Configuration.Application.Handlers.Configuration
{
    ...
    public class DeleteInfoContGasCGDHandler : IRequestHandler<DeleteInfoContGasCGDCommand, ApiResponse<long>>
    {

        private readonly IConfigurationInfrastructure _infrastructure;

        public DeleteInfoContGasCGDHandler(IConfigurationInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(DeleteInfoContGasCGDCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _infrastructure.deleteInfoContGasCGD(request.Data);
