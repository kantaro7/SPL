namespace SPL.Configuration.Application.Handlers.Configuration
{
    ...
    public class SaveCorrectionFactorkWTypeCoolingHandler : IRequestHandler<SaveCorrectionFactorkWTypeCoolingCommand, ApiResponse<long>>
    {

        private readonly IConfigurationInfrastructure _infrastructure;

        public SaveCorrectionFactorkWTypeCoolingHandler(IConfigurationInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveCorrectionFactorkWTypeCoolingCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _infrastructure.saveCorrectionFactorkWTypeCooling(request.Data);
