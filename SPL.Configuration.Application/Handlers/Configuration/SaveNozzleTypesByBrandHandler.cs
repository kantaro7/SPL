namespace SPL.Configuration.Application.Handlers.Configuration
{
    ...
    public class SaveNozzleTypesByBrandHandler : IRequestHandler<SaveNozzleTypesByBrandCommand, ApiResponse<long>>
    {

        private readonly IConfigurationInfrastructure _infrastructure;

        public SaveNozzleTypesByBrandHandler(IConfigurationInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveNozzleTypesByBrandCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var result = await _infrastructure.saveNozzleTypesByBrand(request.Data);
