namespace SPL.Configuration.Application.Handlers.Configuration
{
    ...
    public class SaveInformationOctavesHandler : IRequestHandler<SaveInformationOctavesCommand, ApiResponse<long>>
    {

        private readonly IConfigurationInfrastructure _infrastructure;

        public SaveInformationOctavesHandler(IConfigurationInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveInformationOctavesCommand request, CancellationToken cancellationToken)
        {
            try
            {
