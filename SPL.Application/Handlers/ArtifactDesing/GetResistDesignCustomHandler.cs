...
{
    public class GetResistDesignCustomHandler : IRequestHandler<GetResistDesignCustomQuery, ApiResponse<List<ResistDesign>>>
    {

        private readonly IArtifactdesignInfrastructure _infrastructure;

        public GetResistDesignCustomHandler(IArtifactdesignInfrastructure infrastructure)
        {
            _infrastructure = infrastructure;
        }

        #region Methods
        public async Task<ApiResponse<List<ResistDesign>>> Handle(GetResistDesignCustomQuery request, CancellationToken cancellationToken)
        {
            ...
            if(request.UnitOfMeasurement != "-1")
            {
            ...
            var data = await _infrastructure.GetResistDesignCustom(request.NroSerie, request.UnitOfMeasurement, request.TestConnection, request.Temperature, request.IdSection, request.Order);
            ...
            else
            {
                ...
                {
                    Code = (int)ResponsesID.fallido,
                    Description = "No se ha encontrado información en resistencia de diseño",
                    Structure = data
                };
            }
