...
{
    public class GetCharacterisHandler : IRequestHandler<GetCharacterisQuery, ApiResponse<InfoCarLocal>>
    {

        private readonly IArtifactdesignInfrastructure _infrastructure;

        public GetCharacterisHandler(IArtifactdesignInfrastructure infrastructure)
        {
            _infrastructure = infrastructure;
        }

        #region Methods
        public async Task<ApiResponse<InfoCarLocal>> Handle(GetCharacterisQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (string.IsNullOrEmpty(aparato[0].Trim()))
                {
                    ...
                    return new ApiResponse<InfoCarLocal>()
                    {
                ...
                InfoCarLocal result = await _infrastructure.GetInfoCarLocal(aparato[0]);

                return new ApiResponse<InfoCarLocal>()
                {
                    Code = result == null ? (int)ResponsesID.fallido : (int)ResponsesID.exitoso,
                    Description = result == null ? "El aparato no cuenta con información de las características" : "",
                    Structure = result
                };
            }
            catch (Exception ex)
