namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class IszClientService : IIszClientService
    {
        ...
        public IszClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            try
            {
                ...
                string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.ISZ;
        ...
        public async Task<ApiResponse<long>> SaveReport(ISZTestsGeneralDTO dto)
        {
            try
            {
                UriBuilder uriBuilder = this.GetUriBuilder();
                uriBuilder.Path = _configuration["SaveReportISZ"];
                StringContent dtoJson = new(
