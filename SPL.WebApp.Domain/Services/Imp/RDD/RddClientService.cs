namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class RddClientService : IRddClientService
    {
        private  HttpClient _httpClient;
        private readonly JsonSerializerOptions _options;
        private readonly IConfiguration _configuration;
        private readonly IValidateAccesApis _AAD;
        public RddClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            ...
            string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.RDD;
        ...
        public async Task<ApiResponse<ResultRDDTestsDTO>> CalculateTestRDD(RDDTestsGeneralDTO dto)
        {
            UriBuilder uriBuilder = this.GetUriBuilder();
            uriBuilder.Path = _configuration["CalculateRDD"];
            uriBuilder.Query = string.Empty;
            StringContent dtoJson = new(
                  JsonSerializer.Serialize(dto, _options),
                  Encoding.UTF8,
                  "application/json");
            ApiResponse<ResultRDDTestsDTO> resultRdtTest = new();
            ...
            using (HttpResponseMessage response = await _httpClient.PostAsync(uriBuilder.Uri, dtoJson))
            {
                _ = response.EnsureSuccessStatusCode();
                Stream stream = await response.Content.ReadAsStreamAsync();
                resultRdtTest = await JsonSerializer.DeserializeAsync<ApiResponse<ResultRDDTestsDTO>>(stream, _options);
            }
            return resultRdtTest;
        }

        public async Task<ApiResponse<long>> SaveReport(RDDTestsGeneralDTO dto)
        {
            UriBuilder uriBuilder = this.GetUriBuilder();
            uriBuilder.Path = _configuration["SaveReportRDD"];
            StringContent dtoJson = new(
                  JsonSerializer.Serialize(dto, _options),
                  Encoding.UTF8,
                  "application/json");
        ...
        public async Task<ApiResponse<RDDTestsGeneralDTO>> GetInfoReportRDD(string nroSerie, string keyTest, string lenguage, string unitType, decimal frecuency, bool result)
        {
            UriBuilder uriBuilder = this.GetUriBuilder();
            uriBuilder.Path = this._configuration["GetInfoReportRDD"];
            ...
            _ = response.EnsureSuccessStatusCode();
            Stream stream = await response.Content.ReadAsStreamAsync();
            ApiResponse<RDDTestsGeneralDTO> res = await JsonSerializer.DeserializeAsync<ApiResponse<RDDTestsGeneralDTO>>(stream, this._options);
            return res;
        }
    }
}
