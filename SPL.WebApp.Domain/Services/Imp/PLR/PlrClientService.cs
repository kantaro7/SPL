namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class PlrClientService : IPlrClientService
    {
        ...
        public PlrClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            try
            {
                ...
                string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.PLR;
        ...
        public async Task<ApiResponse<ResultPLRTestsDTO>> CalculateTestPLR(PLRTestsDTO dtos)
        {
            try
            {
                UriBuilder uriBuilder = this.GetUriBuilder();
                uriBuilder.Path = _configuration["CalculatePLR"];
                StringContent dtoJson = new(
                      JsonSerializer.Serialize(dtos, _options),
                      Encoding.UTF8,
                      "application/json");
                ApiResponse<ResultPLRTestsDTO> resultRdtTest = new();
                ...
                using (HttpResponseMessage response = await _httpClient.PostAsync(uriBuilder.Uri, dtoJson))
                {
                    _ = response.EnsureSuccessStatusCode();
                    Stream stream = await response.Content.ReadAsStreamAsync();
                    resultRdtTest = await JsonSerializer.DeserializeAsync<ApiResponse<ResultPLRTestsDTO>>(stream, _options);
                }
        ...
        public async Task<ApiResponse<long>> SaveReport(PLRTestsGeneralDTO dto)
        {
            try
            {
                UriBuilder uriBuilder = this.GetUriBuilder();
                uriBuilder.Path = _configuration["SaveReportPLR"];
                StringContent dtoJson = new(
