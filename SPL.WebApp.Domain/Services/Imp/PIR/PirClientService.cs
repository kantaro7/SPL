namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class PirClientService : IPirClientService
    {
        ...
        public PirClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            try
            {
                ...
                string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.PIR;
        ...
        public async Task<ApiResponse<long>> SaveReport(PIRTestsGeneralDTO dto)
        {
            try
            {
                UriBuilder uriBuilder = this.GetUriBuilder();
                uriBuilder.Path = _configuration["SaveReportPIR"];
                StringContent dtoJson = new(
