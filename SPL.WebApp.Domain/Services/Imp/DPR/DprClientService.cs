namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class DprClientService : IDprClientService
    {
        private  HttpClient _httpClient;
        private readonly JsonSerializerOptions _options;
        private readonly IConfiguration _configuration;
        private readonly IValidateAccesApis _AAD;
        public DprClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            try
            {
                ...
                string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.DPR;
        ...
        public async Task<ApiResponse<ResultDPRTestsDTO>> CalculateTestDPR(DPRTestsGeneralDTO dtos)
        {
            try
            {
                UriBuilder uriBuilder = this.GetUriBuilder();
                uriBuilder.Path = _configuration["CalculateDPR"];
                StringContent dtoJson = new(
                      JsonSerializer.Serialize(dtos, _options),
                      Encoding.UTF8,
                      "application/json");
                ApiResponse<ResultDPRTestsDTO> resultRdtTest = new();
                ...
                using (HttpResponseMessage response = await _httpClient.PostAsync(uriBuilder.Uri, dtoJson))
                {
                    _ = response.EnsureSuccessStatusCode();
                    Stream stream = await response.Content.ReadAsStreamAsync();
                    resultRdtTest = await JsonSerializer.DeserializeAsync<ApiResponse<ResultDPRTestsDTO>>(stream, _options);
                }
        ...
        public async Task<ApiResponse<long>> SaveReport(DPRTestsGeneralDTO dto)
        {
            try
            {
                UriBuilder uriBuilder = this.GetUriBuilder();
                uriBuilder.Path = _configuration["SaveReportDPR"];
                StringContent dtoJson = new(
