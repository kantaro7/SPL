namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class ArfClientService : IArfClientService
    {
        ...
        public ArfClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            try
            {
                ...
                string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.ARF;
        ...
        public async Task<ApiResponse<long>> SaveReport(ARFTestsGeneralDTO dto)
        {
            try
            {
                UriBuilder uriBuilder = this.GetUriBuilder();
                uriBuilder.Path = _configuration["SaveReportARF"];
                StringContent dtoJson = new(
