namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class FpbClientService : IFpbClientService
    {
        ...
        public FpbClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            ...
            string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.FPB;
        ...
        public async Task<ApiResponse<ResultFPBTestsDTO>> CalculateTestFPB(List<FPBTestsDTO> dtos)
        {
            UriBuilder uriBuilder = this.GetUriBuilder();
            uriBuilder.Path = _configuration["CalculateFPB"];
            ...
            ApiResponse<ResultFPBTestsDTO> resultRdtTest = new();
            ...
            using (HttpResponseMessage response = await _httpClient.PostAsync(uriBuilder.Uri, dtoJson))
            {
                ...
                resultRdtTest = await JsonSerializer.DeserializeAsync<ApiResponse<ResultFPBTestsDTO>>(stream, _options);
            }
            return resultRdtTest;
        }

        public async Task<ApiResponse<long>> SaveReport(FPBTestsGeneralDTO dto)
        {
            UriBuilder uriBuilder = this.GetUriBuilder();
            uriBuilder.Path = _configuration["SaveReportFPB"];
            StringContent dtoJson = new(
