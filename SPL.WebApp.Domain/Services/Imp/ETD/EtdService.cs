namespace SPL.WebApp.Domain.Services.Imp.ETD
{
    ...
    public class EtdService : IEtdService
    {
        ...
        public void PrepareUploadConfiguration_ETD(SettingsToDisplayETDReportsDTO reportsDTO, List<bool> listHojas, ref Workbook workbook, string claveIdioma, List<GeneralPropertiesDTO> listIdioma)
        {
            ...
            List<ConfigurationETDReportsDTO> ConfigurationReports = null;
            ...
            foreach (bool item in listHojas)
            {
                ...
                {
                    configF1 = Enumerable.Concat(ConfigurationReports.AsEnumerable(), reportsDTO.ConfigurationReports.Where(x => x.Hoja.Equals("Rep.F1") && x.Hoja.Equals("CortesF1")).AsEnumerable());
                }
                else if (listHojas.IndexOf(item) == 1 && item)
                {
                    configF2 = Enumerable.Concat(ConfigurationReports.AsEnumerable(), reportsDTO.ConfigurationReports.Where(x => x.Hoja.Equals("Rep.F2") && x.Hoja.Equals("CortesF2")).AsEnumerable());
                }
                else
                {
                    if (item)
                        configF3 = Enumerable.Concat(ConfigurationReports.AsEnumerable(), reportsDTO.ConfigurationReports.Where(x => x.Hoja.Equals("Rep.F3") && x.Hoja.Equals("CortesF3")).AsEnumerable());
                }
            }

            IEnumerable<ConfigurationETDReportsDTO> configEstab = Enumerable.Concat(ConfigurationReports.AsEnumerable(), reportsDTO.ConfigurationReports.Where(x => x.Hoja.Equals("Estabilización")).AsEnumerable());
            ...
            else
            {
                ...
                if (listIdioma.FirstOrDefault(x => SinTildes(x.Clave.ToUpper().Trim()).Contains(data)) is not null)
                {
