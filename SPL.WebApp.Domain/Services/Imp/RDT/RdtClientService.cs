namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class RdtClientService : IRdtClientService
    {
        private  HttpClient _httpClient;
        private readonly JsonSerializerOptions _options;
        private readonly IConfiguration _configuration;
        private readonly IValidateAccesApis _AAD;
        public RdtClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            ...
            string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.RDT;
        ...
        public async Task<ApiResponse<ResultRDTTestsDetailsDTO>> CalculateTestRDT(RDTTestsDTO dto)
        {
            UriBuilder uriBuilder = this.GetUriBuilder();
            uriBuilder.Path = _configuration["CalculateRDT"];
            uriBuilder.Query = string.Empty;
            StringContent dtoJson = new(
                  JsonSerializer.Serialize(dto, _options),
                  Encoding.UTF8,
                  "application/json");
            ApiResponse<ResultRDTTestsDetailsDTO> resultRdtTest = new();
            ...
            using (HttpResponseMessage response = await _httpClient.PostAsync(uriBuilder.Uri, dtoJson))
            {
                _ = response.EnsureSuccessStatusCode();
                Stream stream = await response.Content.ReadAsStreamAsync();
                resultRdtTest = await JsonSerializer.DeserializeAsync<ApiResponse<ResultRDTTestsDetailsDTO>>(stream, _options);
            }
            return resultRdtTest;
        }

        public async Task<ApiResponse<long>> SaveReport(RDTReportDTO dto)
        {
            UriBuilder uriBuilder = this.GetUriBuilder();
            uriBuilder.Path = _configuration["SaveReportRDT"];
            StringContent dtoJson = new(
                  JsonSerializer.Serialize(dto, _options),
                  Encoding.UTF8,
                  "application/json");
