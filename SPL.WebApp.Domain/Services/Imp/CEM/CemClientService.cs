namespace SPL.WebApp.Domain.Services.Imp
{
    ...
    public class CemClientService : ICemClientService
    {
        ...
        public CemClientService(HttpClient httpClient, IConfiguration config, IValidateAccesApis aad)
        {
        ...
        public async Task<ApiResponse<List<InfoGeneralTypesReportsDTO>>> GetFilter(string nroSerie)
        {
            try
            {
                ...
                string url = uriBuilder.Uri + "/" + nroSerie + "/" + ReportType.CEM;
        ...
        public async Task<ApiResponse<long>> SaveReport(CEMTestsGeneralDTO dto)
        {
            try
            {
                UriBuilder uriBuilder = this.GetUriBuilder();
                uriBuilder.Path = _configuration["SaveReportCEM"];
        ...
        public async Task<ApiResponse<CEMTestsGeneralDTO>> GetInfoReportCEM(string nroSerie, string keyTest, bool result)
        {
            UriBuilder uriBuilder = this.GetUriBuilder();
            uriBuilder.Path = this._configuration["GetInfoReportCEM"];
            ...
            ApiResponse<CEMTestsGeneralDTO> infoGnralTypesReports = await JsonSerializer.DeserializeAsync<ApiResponse<CEMTestsGeneralDTO>>(stream, this._options);
