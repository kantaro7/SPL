namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextPEEHandler : IRequestHandler<GetNumNextPEEQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextPEEHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextPEEQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextPEE(request.NroSerie, request.KeyTests, request.Lenguage);
