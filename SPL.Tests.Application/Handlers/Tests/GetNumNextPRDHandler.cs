namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextPRDHandler : IRequestHandler<GetNumNextPRDQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextPRDHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextPRDQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextPRD(request.NroSerie, request.KeyTests, request.Lenguage, request.NominalVoltage);
