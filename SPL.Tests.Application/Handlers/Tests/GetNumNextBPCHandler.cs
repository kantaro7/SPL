namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextBPCHandler : IRequestHandler<GetNumNextBPCQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextBPCHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextBPCQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextBPC(request.NroSerie, request.KeyTests, request.Lenguage);
