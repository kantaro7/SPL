namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextRDDHandler : IRequestHandler<GetNumNextRDDQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextRDDHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextRDDQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextRDD(request.NroSerie, request.KeyTests, request.Lenguage, request.Config_Winding, request.Connection, request.Porc_Z, request.Porc_Jx);
