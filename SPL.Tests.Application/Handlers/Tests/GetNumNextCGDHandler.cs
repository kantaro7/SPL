namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextCGDHandler : IRequestHandler<GetNumNextCGDQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextCGDHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextCGDQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextCGD(request.NroSerie, request.KeyTests, request.Lenguage, request.TypeOil);
