namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextCEMHandler : IRequestHandler<GetNumNextCEMQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextCEMHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextCEMQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextCEM(request.NroSerie, request.KeyTests, request.Lenguage, request.IdPosPrimary, request.PosPrimary, request.IdPosSecundary, request.PosSecundary, request.TestsVoltage);
