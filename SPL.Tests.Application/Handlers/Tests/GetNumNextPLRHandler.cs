namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextPLRHandler : IRequestHandler<GetNumNextPLRQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextPLRHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextPLRQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextPLR(request.NroSerie, request.KeyTests, request.Lenguage, request.Rldnt, request.NominalVoltage, request.AmountOfTensions);
