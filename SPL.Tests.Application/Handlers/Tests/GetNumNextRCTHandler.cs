namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextRCTHandler : IRequestHandler<GetNumNextRCTQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextRCTHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextRCTQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                //if (string.IsNullOrEmpty(request.Tertiary))
                //{
                //    return new ApiResponse<long>()
                //    {
                //        Code = (int)ResponsesID.fallido,
                //        Description = "Terciario o segunda baja es requerido",
                //        Structure = 0
                //    };
                //}
                ...
                long result = await this._infrastructure.GetNumTestNextRCT(request.NroSerie, request.KeyTests, request.Lenguage, request.UnitOfMeasurement, request.Tertiary, request.Testvoltage);
