namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextFPAHandler : IRequestHandler<GetNumNextFPAQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextFPAHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextFPAQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextFPA(request.NroSerie, request.KeyTests, request.Lenguage, request.TypeOil);
