namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextPCEHandler : IRequestHandler<GetNumNextPCEQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextPCEHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextPCEQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(request.EnergizedWinding))
                {
                    return new ApiResponse<long>()
                    {
                        Code = (int)ResponsesID.fallido,
                        Description = "Devanado energizado es requerido",
                        Structure = 0
                    };
                }
                ...
                long result = await this._infrastructure.GetNumTestNextPCE(request.NroSerie, request.KeyTests, request.Lenguage, request.EnergizedWinding);
