namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextFPBHandler : IRequestHandler<GetNumNextFPBQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextFPBHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextFPBQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(request.TangentDelta))
                {
                    return new ApiResponse<long>()
                    {
                        Code = (int)ResponsesID.fallido,
                ...
                long result = await this._infrastructure.GetNumTestNextFPB(request.NroSerie, request.KeyTests, request.Lenguage, request.TangentDelta);
