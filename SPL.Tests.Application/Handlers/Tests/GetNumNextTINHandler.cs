namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextTINHandler : IRequestHandler<GetNumNextTINQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextTINHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextTINQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextTIN(request.NroSerie, request.KeyTests, request.Lenguage, request.Connection, request.Voltage);
