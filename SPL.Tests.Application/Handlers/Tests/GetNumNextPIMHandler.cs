namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextPIMHandler : IRequestHandler<GetNumNextPIMQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextPIMHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextPIMQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextPIM(request.NroSerie, request.KeyTests, request.Lenguage, request.Connection, request.ApplyLow, request.VoltageLevel);
