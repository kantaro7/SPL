namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextINDHandler : IRequestHandler<GetNumNextINDQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextINDHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextINDQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextIND(request.NroSerie, request.KeyTests, request.Lenguage, request.TcPurchased);
