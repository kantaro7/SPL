namespace SPL.Tests.Application.Handlers.Tests
{
    ...
    public class GetNumNextNRAHandler : IRequestHandler<GetNumNextNRAQuery, ApiResponse<long>>
    {

        private readonly ITestsInfrastructure _infrastructure;

        public GetNumNextNRAHandler(ITestsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(GetNumNextNRAQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                long result = await this._infrastructure.GetNumTestNextNRA(request.NroSerie, request.KeyTests, request.Language,request.Laboratory, request.Rule, request.Feeding, request.CoolingType);
