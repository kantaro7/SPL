...
btnSave.addEventListener("click", function () {
    ...
    postData(domain + "/Pee/SavePDF/", viewModel)
        .then(data => {
...
btnValidate.addEventListener("click", function () {
    ...
    calculado = true;
    var data = viewModel;
    postData(domain + "/Pee/ValidateData/", data)
        .then(data => {
            if (data.response.Code === -2 || data.response.Code ===5) {
                ShowFailedMessage(data.response.Description);
                btnRefresh.disabled = false;
                ...
                if (data.response.Description !== '') {
                    ShowWarningMessage(data.response.Description);
                }
            }
            else {
                viewModel = data.response.Structure;
