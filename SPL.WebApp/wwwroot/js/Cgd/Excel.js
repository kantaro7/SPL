...
btnSave.addEventListener("click", function () {
    if (calculado) {
        ...
        postData(domain + "/Cgd/SavePDF/", viewModel)
            .then(data => {
...
btnValidate.addEventListener("click", function () {
    ...
    calculado = true;
    var data = viewModel;
    postData(domain + "/Cgd/ValidateData/", data)
        .then(data => {
...
function SetConfigExcel(workbook) {
    ...
    var range;
    if (viewModel.ClavePrueba == "DDT") {
        range = (viewModel.TemperatureHour3 != 0) ? spreadsheet.activeSheet().range("A175:J200") : (viewModel.TemperatureHour2 != 0) ? spreadsheet.activeSheet().range("A117:J200") : spreadsheet.activeSheet().range("A60:J200");
    } else {
        range = spreadsheet.activeSheet().range("A60:J200");
    }
    range.enable(false);
    initCss();
}
