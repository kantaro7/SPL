...
let nominalVoltage = document.getElementById("NominalVoltage");
...
async function ValidateFilterJSON() {
    var path = path = "/Prd/ValidateFilter/";

    var url = new URL(domain + path),
        params = { noSerie: noSerieInput.value, clavePrueba: clavePruebaInput.value, claveIdioma: claveIdiomaInput.value, nominalVoltage: nominalVoltage.value, comment: CommentsInput.value }
...
async function GetPDFJSON(code, tyoeReport) {
    var path = path = "/Prd/GetPDFReport/";
...
function MapToViewModel(loadWorkbook = false, loadOfficial = false) {
    ...
    viewModel.NominalVoltage = nominalVoltage.value;
