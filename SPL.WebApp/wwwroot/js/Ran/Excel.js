...
btnSave.addEventListener("click", function () {
    ...
    LoadWorkbook();
    postData(domain + "/Ran/SavePDF/", viewModel)
        .then(data => {
...
btnValidate.addEventListener("click", function () {
    $("#loader").css("display", "block");
    LoadWorkbook();
    postData(domain + "/Ran/Validate/", viewModel)
        .then(data => {
            if (data.response.Code !== -1) {
...
function SetDataSourceSpredsheet(workbook) {
    spreadsheetElementRan.data("kendoSpreadsheet").destroy()
    $("#spreadsheet").empty();
    SetConfigExcel(workbook);
}
...
function SetConfigExcel(workbook) {
    ...
    spreadsheetElementRan = $("#spreadsheet").kendoSpreadsheet({
