...
measuringInput.addEventListener("blur", function () {

    ValidateMeasuring();

});

clavePruebaInput.addEventListener("blur", function () {
...
async function GetTemplateJSON() {
    var path = path = "/Ran/GetTemplate/";

    var url = new URL(domain + path),
        params = { noSerie: noSerieInput.value, clavePrueba: clavePruebaInput.value, claveIdioma: claveIdiomaInput.value, measuring: parseInt(measuringInput.value), comment: CommentsInput.value }
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
    $("#loader").css("display", "none");
...
function ValidateMeasuring() {
    if (measuringInput.value === "" || measuringInput.value === "0") {
    ...
    switch (clavePruebaInput.value) {
        case "AYD":
            ...
            if (!(measunring >= 1 && measunring <= 9)) {
                ShowFailedMessage("La prueba solo acepta de 1 hasta 9 mediciones, favor de corregirlo.");
        ...
        case "APD":
            ...
            if (!(measunring >= 1 && measunring <= 14)) {
                ShowFailedMessage("La prueba solo acepta de 1 hasta 14 mediciones, favor de corregirlo.");
                return false;
            }
            break;
        default:
...
async function GetPDFJSON(code, tyoeReport) {
    var path = path = "/Ran/GetPDFReport/";
...
function MapToViewModel(loadWorkbook = false) {
    ...
    viewModel.Measuring = measuringInput.value;
