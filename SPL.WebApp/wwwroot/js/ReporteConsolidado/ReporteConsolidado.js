...
$("#NoSerie").on('change', async function (e) {
    $("#loader").css("display", "initial");
    btnRequest.disabled = true
    btnClear.disabled = true 
    if (noSerieInput.value) {
        await GetIdioma().then(
            data => {
                if (data.Code !== -1) {
                    viewModel = data.Structure;
                    claveIdiomaInput.value = viewModel.ClaveIdioma
                }
                else {
...
function LoadTreeView(treeViewModel) {
   ...
   $.each(viewModel.datasource, function (key, val) {
        ...
        let insertar = {
            ID_REP: 999,
            TIPO_REPORTE: 'Nombre del Reporte',
            ID_PRUEBA: "Prueba",
...
btnRequest.addEventListener("click", function () {
    ...
    if (claveIdiomaInput.value === '') {
        $(`#ClaveIdiomaSpand`).text("Requerido");
        return
    } else {
        $(`#ClaveIdiomaSpand`).text("");
    }
...
btnLoadTemplate.addEventListener("click", function () {
    ...
    postData(domain + "/ReporteConsolidado/GetReports/", viewModel)
        .then(data => {
            console.log(data)
            if (data.response.Code !== -1) {
                ShowSuccessMessage("Exportando reporte consolidado.")
                LoadPDF(noSerieInput.value +claveIdiomaInput.value+".docx", data.response.Structure);
                btnLoadTemplate.disabled = false;
                btnClear.disabled = false
                $("#loader").css("display", "none");
            }
...
async function CheckInfo() {
    var path = path = "/ReporteConsolidado/CheckInfo/";

    var url = new URL(domain + path),
        params =
        {
            noSerie: noSerieInput.value,
            idioma: claveIdiomaInput.value,
        }
...
async function GetIdioma() {
    var path = path = "/ReporteConsolidado/GetIdioma/";
...
function LoadPDF(nameFile, File) {
    ...
    var file = new Blob([byteArray], { type: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64' });
