...
//Variables
let viewModel;
let settingsToDisplayFPBReportsDTO;
...
let tandeltaInput = document.getElementById("TanDelta");
...
document.querySelector('#btnInfo').addEventListener('click', () => {
    var text =
        ...
        + '<ul>'
        + '<li>Antes y Después de Pruebas (Ambos Idiomas)</li>'
        + '<li>Antes de Pruebas (Ambos Idiomas)</li>'
        + '<li>Después de Pruebas (Ambos Idiomas)</li>'
        + '</ul>'
        + '<br>'
        + 'Se debe de contar con Información registrada del aparato en la pantalla: “Información de Boquillas”.';
...
async function GetTemplateJSON() {
    var path = path = "/Fpb/GetTemplate/";
    var url = new URL(domain + path),
        params = { noSerie: noSerieInput.value, clavePrueba: clavePruebaInput.value, claveIdioma: claveIdiomaInput.value, tangtDelta: tandeltaInput.value, comment: CommentsInput.value }
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
    $("#loader").css("display", "none");
...
function MapToViewModel(loadWorkbook = false, loadOfficial = false) {
    ...
    viewModel.TanDelta = tandeltaInput.value;
...
async function GetPDFJSON(code, tyoeReport) {
    var path = path = "/Fpb/GetPDFReport/";
