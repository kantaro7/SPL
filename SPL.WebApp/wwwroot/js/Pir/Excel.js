...
btnValidate.addEventListener("click", async function () {
    ...
    if (configuraciones === null || configuraciones === undefined) {
        ShowFailedMessage("No se ha podido cargar la configuracion de archivos para PIR")
...
for(const elemento of files){
    ...
    $("#loader").css("display", "block");
    postData(domain + "/Pir/SavePDF/", viewModel)
        .then(data => {
...
function SetConfigExcel(workbook) {
    ...
    kendo.spreadsheet.defineFunction("VALIDATE_RELTENSION", function(str, pattern1, pattern2 ,flags){
        var rx;
        var test1;
        try {
            if(str.includes('-')){
                ...
                if(sp.length == 2 )
                {
                    rx = flags ? new RegExp(pattern1, flags) : new RegExp(pattern1);
                    test1 = rx.test(str)
                ...
                rx = flags ? new RegExp(pattern2, flags) : new RegExp(pattern2);
                test1 = rx.test(str)
                return test1
            }
    ...
    for(var i = 14; i<=(14 + viewModel.Contador - 1);i++){
        sheet.range("G"+i).validation({
            comparerType: "custom",
            dataType: "custom",
            from: 'AND(VALIDATE_RELTENSION(G'+i+', "^\\\\d+(?:-\\\\d+)$","^[0-9]*$"),LEN(G'+i+')<=10)',
            titleTemplate : "Error",
            showButton : true,
            type : "reject",
            allowNulls : false,
            messageTemplate : "La página solo puede contener números y guion medio y su contenido no puede ser mayor a 10 caracteres"
        });
