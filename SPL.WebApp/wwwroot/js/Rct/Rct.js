
//Variables
let viewModel;
let tensionTestRequired = false;
let thirdSecondDownRequired = false;
let btnRequest = document.getElementById("btnRequest");
let btnClear = document.getElementById("btnClear");
let btnLoadTemplate = document.getElementById("btnLoadTemplate");
let noSerieInput = document.getElementById("NoSerie");
let CommentsInput = document.getElementById("Comments");
let claveIdiomaInput = document.getElementById("ClaveIdioma");
let unitMeasuringInput = document.getElementById("UnitMeasuring");
let thirdSecondDownInput = document.getElementById("ThirdSecondDown");
let tensionTestInput = document.getElementById("TensionTest");
let noPruebaInput = document.getElementById("NoPrueba");
let clavePruebaInput = document.getElementById("ClavePrueba");
let spreadsheetElementRCT;
let treeViewKendoElement;
let panelExcel = document.getElementById("spreadsheet");
let validMeasuring = true;

//Mostrar info necesaria para consultar el reporte
document.querySelector('#btnInfo').addEventListener('click', () => {
    var text =
        'Considerar que para este reporte:<br><br>'
        + 'El “No. De Serie” del Aparato se debe de encontrar registrado en SPL.<br>'
        + 'Se debe contar con Información en la pantalla: “Diseño del Aparato”.<br>'
        + 'Se deben de contar con las plantillas cargadas para las pruebas correspondientes del reporte:<br>'
        + '<ul>'
        + '<li>Alta Tensión y Baja Tensión Individuales (Ambos Idiomas)</li>'
        + '<li>Alta Tensión y Baja Tensión Simultáneos (Ambos Idiomas)</li>'
        + '<li>Alta Tensión Individual (Ambos Idiomas)</li>'
        + '<li>Alta Tensión Individual, Baja Tensión y Terciario Simultáneos (Ambos Idiomas)</li>'
        + '<li>Alta Tensión, Baja Tensión y Terciario Individuales (Ambos Idiomas)</li>'
        + '<li>Alta Tensión, Baja Tensión y Terciario Simultáneos (Ambos Idiomas)</li>'
        + '<li>Alta Tensión, Baja Tensión Simultáneos y Terciario Individual (Ambos Idiomas)</li>'
        + '</ul>';
    ShowWarningMessageHtmlContent(text);
});

//Evento
$("#NoSerie").focus();
$("#NoSerie").on('keyup', function (e) {
    if (e.key === 'Enter' || e.keyCode === 13) {
        $("#btnRequest").click();
    }
});

tensionTestInput.addEventListener("blur", function (event) {
    if (tensionTestRequired && tensionTestInput.value === '') {
        $(`#TensionTestSpand`).text("Requerido");
    }
    else {
        $(`#TensionTestSpand`).text("");
    }
});

$("#ClavePrueba").on("change", function () {
    contador = 0;
    if (clavePruebaInput.value !== '') {
        $("#TensionTest option[value!='']").each(function () {
            $(this).remove();
        });
        var length = TensionTest.options.length;
        for (i = length - 1; i > 0; i--) {
            tensionTestInput.options[i] = null;
        }

        if (clavePruebaInput.value == "ATI") {
            for (var i = 0; i < viewModel.AT.length; i++) {
                $("#TensionTest").append("<option value='" + viewModel.AT[i] + "'>" + viewModel.AT[i] + "</option>")
            }

        } else if (clavePruebaInput.value == "ABI" || clavePruebaInput.value == "ABS"){
            for (var i = 0; i < viewModel.ABT.length; i++) {
                $("#TensionTest").append("<option value='" + viewModel.ABT[i] + "'>" + viewModel.ABT[i] + "</option>")
            }
        } else {
            for (var i = 0; i < viewModel.ABTER.length; i++) {
                $("#TensionTest").append("<option value='" + viewModel.ABTER[i] + "'>" + viewModel.ABTER[i] + "</option>")
            }
        }
    } else {
        $("#TensionTest option[value!='']").each(function () {
            $(this).remove();
        });
        var length = TensionTest.options.length;
        for (i = length - 1; i > 0; i--) {
            tensionTestInput.options[i] = null;
        }
    }
});

btnClear.addEventListener("click", function () {

    noSerieInput.value = '';
    //noPruebaInput.value = '';
    CommentsInput.value = '';
    claveIdiomaInput.value = '';
    clavePruebaInput.value = '';
    unitMeasuringInput.value = '';
    thirdSecondDownInput.value = '';
    tensionTestInput.value = '';

    btnRequest.disabled = false;
    btnClear.disabled = false;
    btnLoadTemplate.disabled = true;
    noSerieInput.disabled = false;

    claveIdiomaInput.disabled = false;
    clavePruebaInput.disabled = false;
    $(`#ThirdSecondDownSpand`).text("");
    if (treeViewKendoElement !== undefined) {
        kendo.destroy(treeViewKendoElement);
        $("#treeview-kendo").empty();
    }

    panelPDF.style.display = "none";


    var length = clavePruebaInput.options.length;
    for (i = length - 1; i > 0; i--) {
        clavePruebaInput.options[i] = null;
    }

});

btnLoadTemplate.addEventListener("click", function () {

    if (ValidateForm()) {
        MapToViewModel();
        $("#loader").css("display", "block");
        btnRequest.disabled = true;
        btnClear.disabled = false;
        btnLoadTemplate.disabled = false;

        ValidateFilterJSON(null).then(
            data => {
                if (data.response.Code !== -1) {
                    GetTemplateJSON();
                }
                else {
                    ShowFailedMessage(data.response.Description);
                }
            }
        );

    }
    else {
        $("#loader").css("display", "none");
        ShowFailedMessage("Hay campos requeridos, por favor verifique.");
    }
});

btnRequest.addEventListener("click", function () {

    if (noSerieInput.value === undefined || noSerieInput.value === "" || noSerieInput.value === null) {
        $(`#NoSerieSpand`).text("Requerido");
        return;
    }
    else {
        $(`#NoSerieSpand`).text("");
    }

    if (!(/^[A-Za-zäÄëËïÏöÖüÜáéíóúáéíóúÁÉÍÓÚÂÊÎÔÛâêîôûàèìòùÀÈÌÒÙ0-9-@]*$/.test(noSerieInput.value))) {
        $(`#NoSerieSpand`).text("Character(es) no permitido.");
        return;
    }
    else {
        $(`#NoSerieSpand`).text("");
    }

    $("#loader").css("display", "block");
    if (noSerieInput.value) {

        GetFilterJSON(null).then(
            data => {
                if (data.response.Code !== -1) {
                    noSerieInput.disabled = true;

                    var length = clavePruebaInput.options.length;
                    for (i = length - 1; i > 0; i--) {
                        clavePruebaInput.options[i] = null;
                    }


                    let listOptions = data.response.Structure.Tests;
                    var opt;

                    listOptions.forEach(function (data, key) {

                        opt = document.createElement("option");
                        opt.value = data.Clave;
                        opt.innerHTML = data.Descripcion;

                        clavePruebaInput.appendChild(opt);
                    });

                    viewModel = data.response.Structure;
                    LoadTreeView(viewModel.TreeViewItem);
                    LoadForm(viewModel);

                    $("#UnitMeasuring").val("Ohms");

                    btnRequest.disabled = true;
                    btnClear.disabled = false;
                    btnLoadTemplate.disabled = false;
                }
                else {
                    ShowFailedMessage(data.response.Description);

                    $("#loader").css("display", "none");
                }

                $("#loader").css("display", "none");
            }
        );
    }
    else {

        ShowSuccessMessage('Por favor ingrese un No. Serie.');
        btnSave.disabled = true;
        btnLoadDataSidco.disabled = true;
        $("#loader").css("display", "none");
    }

});

//Requests

async function ValidateFilterJSON() {
    var path = path = "/Rct/ValidateFilter/";

    var url = new URL(domain + path),
        params = { noSerie: noSerieInput.value, clavePrueba: clavePruebaInput.value, claveIdioma: claveIdiomaInput.value, terciario: thirdSecondDownInput.value, unitOfMeasurement: unitMeasuringInput.value, testVoltage: tensionTestInput.value, comment: CommentsInput.value }
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))

    const response = await fetch(url);

    if (response.ok && response.status === 200) {

        const result = await response.json();
        return result;
    }

    else {
        ShowFailedMessage('Error, por favor contacte al administrador del sistema.');
        return null;
    }
}

async function GetTest() {
    var path = path = "/Rct/GetTest/";

    var url = new URL(domain + path),
        params = { noSerie: noSerieInput.value }
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))

    const response = await fetch(url);

    if (response.ok && response.status === 200) {

        const ResistanceTwentyDegrees = await response.json();
        return ResistanceTwentyDegrees;
    }

    else {
        ShowFailedMessage('Error, por favor contacte al administrador del sistema.');
        return null;
    }
}

async function GetFilterJSON() {
    var path = path = "/Rct/GetFilter/";

    var url = new URL(domain + path),
        params = { noSerie: noSerieInput.value }
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))

    const response = await fetch(url);

    if (response.ok && response.status === 200) {

        const result = await response.json();
        return result;
    }

    else {
        ShowFailedMessage('Error, por favor contacte al administrador del sistema.');
        return null;
    }
}

async function GetTemplateJSON() {
    var path = path = "/Rct/GetTemplate/";

    var url = new URL(domain + path),
        params = { noSerie: noSerieInput.value, clavePrueba: clavePruebaInput.value, claveIdioma: claveIdiomaInput.value, terciario: thirdSecondDownInput.value, unitOfMeasurement: unitMeasuringInput.value, testVoltage: tensionTestInput.value, comment: CommentsInput.value }
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))
    $("#loader").css("display", "none");
    var win = window.open(url.toString(), '_blank');
    win.focus();
}

//Functions

function ValidateFilter() {
    let filterValid = true;

    if (clavePruebaInput.value === '')
        filterValid = false;

    if (unitMeasuringInput.value === '')
        filterValid = false;

    //if (thirdSecondDownInput.value === '')
    //    filterValid = false;

    if (claveIdiomaInput.value === '')
        filterValid = false;

    if (TensionTest.value === '' && tensionTestRequired) {
        filterValid = false;
        $(`#TensionTestSpand`).text("Requerido");
    }
    else {
        $(`#TensionTestSpand`).text("");
    }

    if (thirdSecondDownInput.value === '' && thirdSecondDownRequired) {
        filterValid = false;
        $(`#ThirdSecondDownSpand`).text("Requerido");
    }
    else {
        $(`#ThirdSecondDownSpand`).text("");
    }

    return filterValid;

}

function ValidateMeasuring() {
    if (measuringInput.value === "" || measuringInput.value === "1") {
        ShowFailedMessage("Favor de indicar la cantidad de mediciones.");

        return false;
    }

    switch (clavePruebaInput.value) {
        case "AYD":
            var measunring = parseInt(measuringInput.value);

            if (!(measunring >= 2 && measunring <= 6)) {
                ShowFailedMessage("La prueba solo acepta de 2 hasta 6 mediciones, favor de corregirlo.");

                return false;
            }
            break;
        case "APD":
            var measunring = parseInt(measuringInput.value);

            if (!(measunring >= 2 && measunring <= 14)) {
                ShowFailedMessage("La prueba solo acepta de 2 hasta 14 mediciones, favor de corregirlo.");
                return false;
            }
            break;
        default:
            break;
    }
    return true;
}

function LoadPDF(nameFile, File) {

    var byteCharacters = atob(File);
    var byteNumbers = new Array(byteCharacters.length);
    for (var i = 0; i < byteCharacters.length; i++) {
        byteNumbers[i] = byteCharacters.charCodeAt(i);
    }
    var byteArray = new Uint8Array(byteNumbers);
    var file = new Blob([byteArray], { type: 'application/pdf;base64' });
    var fileURL = URL.createObjectURL(file);
    window.open(fileURL);
}

function showPdfInNewTab(base64Data, fileName) {

    let pdfWindow = window.open("");
    pdfWindow.document.write("<html<head><title>" + fileName + "</title><style>body{margin: 0px;}iframe{border-width: 0px;}</style></head>");
    pdfWindow.document.write("<body><embed width='100%' height='100%' src='data:application/pdf;base64, " + encodeURI(base64Data) + "#toolbar=0&navpanes=0&scrollbar=0'></embed></body></html>");
}

function LoadTreeView(treeViewModel) {

    treeViewKendoElement = $("#treeview-kendo").kendoTreeView({
        template: kendo.template($("#treeview").html()),
        dataSource: treeViewModel,
        dragAndDrop: false,
        checkboxes: false,
        select: onSelect,
        loadOnDemand: true
    });

}

function onSelect(e) {
    var text = this.text(e.node);
    if (text.split('.').length > 1) {
        var id = text.split('.')[0].split('-')[2].split('_')[1];
        console.log("Selecting: " + this.text(e.node));
        console.log(id);


        GetPDFJSON(id, "RCT").then(
            data => {
                if (data != null) {

                    LoadPDF(this.text(e.node), data.data);
                }
                else {
                    ShowFailedMessage(data.response.Description);
                }
                $("#loader").css("display", "none");
            }
        );
    }

}

async function GetPDFJSON(code, tyoeReport) {
    var path = path = "/Rct/GetPDFReport/";

    var url = new URL(domain + path),
        params = { code: code, typeReport: tyoeReport }
    Object.keys(params).forEach(key => url.searchParams.append(key, params[key]))

    const response = await fetch(url);

    if (response.ok && response.status === 200) {

        const result = await response.json();
        return result;
    }

    else {
        ShowFailedMessage('Error, por favor contacte al administrador del sistema.');
        return null;
    }
}

function LoadForm(response) {
    claveIdiomaInput.value = response.ClaveIdioma;

    thirdSecondDownInput.value = response.ThirdSecondDown;
    tensionTestRequired = response.TensionTestRequired;
    thirdSecondDownRequired = response.ThirdSecondDownRequired;
}

function ValidateForm() {



    var ResultValidations = $("#form_menu_sup").data("kendoValidator").validate();

    if (thirdSecondDownInput.value === '' && thirdSecondDownRequired) {
        ResultValidations = false;
        $(`#ThirdSecondDownSpand`).text("Requerido");
    }
    else {
        $(`#ThirdSecondDownSpand`).text("");
    }


    if (ResultValidations) {
        return true;
    }
    else {
        return false;
    }

}

function MapToViewModel(loadWorkbook = false) {

    viewModel.NoSerie = noSerieInput.value;
    viewModel.ClaveIdioma = claveIdiomaInput.value;
    viewModel.ClavePrueba = clavePruebaInput.value;
    viewModel.UnitMeasuring = unitMeasuringInput.value;
    viewModel.ThirdSecondDown = thirdSecondDownInput.value;
    viewModel.TensionTest = tensionTestInput.value;

    //viewModel.NoPrueba = noPruebaInput.value;
    viewModel.Comments = CommentsInput.value;

    if (loadWorkbook)
        viewModel.Workbook.sheets = $("#spreadsheet").getKendoSpreadsheet().toJSON().sheets;

}
