namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportDPRHandler : IRequestHandler<SaveReportDPRCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportDPRHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportDPRCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoDPRReport(request.Data);
