namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportPRDHandler : IRequestHandler<SaveReportPRDCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportPRDHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportPRDCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoPRDReport(request.Data);
