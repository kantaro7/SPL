namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportINDHandler : IRequestHandler<SaveReportINDCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportINDHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportINDCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoINDReport(request.Data);
