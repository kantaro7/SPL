namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportRDDHandler : IRequestHandler<SaveReportRDDCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportRDDHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportRDDCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoRDDReport(request.Data);
