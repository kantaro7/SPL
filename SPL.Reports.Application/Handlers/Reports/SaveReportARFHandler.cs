namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportARFHandler : IRequestHandler<SaveReportARFCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportARFHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportARFCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoARFReport(request.Data);
