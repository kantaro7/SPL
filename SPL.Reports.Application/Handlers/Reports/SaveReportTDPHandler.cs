namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportTDPHandler : IRequestHandler<SaveReportTDPCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportTDPHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportTDPCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoTDPReport(request.Data);
