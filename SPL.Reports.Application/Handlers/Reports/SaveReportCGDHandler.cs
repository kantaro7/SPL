namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportCGDHandler : IRequestHandler<SaveReportCGDCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportCGDHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportCGDCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoCGDReport(request.Data);
