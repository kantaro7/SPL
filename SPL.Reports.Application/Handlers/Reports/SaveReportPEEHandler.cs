namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportPEEHandler : IRequestHandler<SaveReportPEECommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportPEEHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportPEECommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoPEEReport(request.Data);
