namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportPIRHandler : IRequestHandler<SaveReportPIRCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportPIRHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportPIRCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoPIRReport(request.Data);
