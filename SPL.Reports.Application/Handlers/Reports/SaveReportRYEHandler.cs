namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportRYEHandler : IRequestHandler<SaveReportRYECommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportRYEHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportRYECommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (!request.Data.CoolingType.Contains("ONAN") && request.Data.Data.LostCooldown == 0)
                {
                    return new ApiResponse<long>()
                    {
                        Code = (int)ResponsesID.fallido,
                ...
                long result = await this._infrastructure.SaveInfoRYEReport(request.Data);
