namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportBPCHandler : IRequestHandler<SaveReportBPCCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportBPCHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportBPCCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                long result = await this._infrastructure.SaveInfoBPCReport(request.Data);
