namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportPCEHandler : IRequestHandler<SaveReportPCECommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportPCEHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportPCECommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                foreach (PCETests item in request.Data.Data)
                {
                ...
                long result = await this._infrastructure.SaveInfoPCEReport(request.Data);
