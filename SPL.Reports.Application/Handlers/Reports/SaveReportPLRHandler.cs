namespace SPL.Reports.Application.Handlers.Reports
{
    ...
    public class SaveReportPLRHandler : IRequestHandler<SaveReportPLRCommand, ApiResponse<long>>
    {

        private readonly IReportsInfrastructure _infrastructure;

        public SaveReportPLRHandler(IReportsInfrastructure infrastructure) => this._infrastructure = infrastructure;

        #region Methods
        public async Task<ApiResponse<long>> Handle(SaveReportPLRCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                int[] ValidPorcDeviationNV = CommonMethods.cantDigitsPoint(Math.Abs(Convert.ToDouble(request.Data.Data.PorcDeviationNV)));
                if (ValidPorcDeviationNV[0] > 3 || ValidPorcDeviationNV[1] > 4)
                {
                    return new ApiResponse<long>()
                    {
                        Code = (int)ResponsesID.fallido,
                        Description = "No se puede realizar el registro porque el valor PorcDeviationNV " + request.Data.Data.PorcDeviationNV + " supera el limite en BD, que es numeric(7,4). Por favor revise los datos y vuelva a calcular",
                        Structure = -1
                    };
                }


                long result = await this._infrastructure.SaveInfoPLRReport(request.Data);
