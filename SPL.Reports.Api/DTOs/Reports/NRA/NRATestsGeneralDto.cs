using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.Linq;
using System.Text;
using System.Threading.Tasks;

namespace SPL.Reports.Api.DTOs.Reports.NRA
{
   public class NRATestsGeneralDto
    {

        public long IdLoad { get; set; }
        public DateTime LoadDate { get; set; }

        [Range(0, 99, ErrorMessage = "El campo Número de Prueba debe ser numérico mayor a cero considerando hasta 2 enteros")]
        public int TestNumber { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(2, ErrorMessage = "El campo Clave Idioma solo puede tener {0} caracteres")]
        public string LanguageKey { get; set; }


        [DataType(DataType.Text)]
        [MaxLength(512, ErrorMessage = "El campo Cliente solo puede tener {0} caracteres")]
        public string Customer { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(512, ErrorMessage = "El campo Capacidad solo puede tener {0} caracteres")]
        public string Capacity { get; set; }


        [DataType(DataType.Text)]
        [MaxLength(55, ErrorMessage = "El campo Número de Serie solo puede tener {0} caracteres")]
        public string SerialNumber { get; set; }



        public bool Result { get; set; }
        public string NameFile { get; set; }
        public byte[] File { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(3, ErrorMessage = "El campo Tipo Reporte solo puede tener {0} caracteres")]
        public string TypeReport { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(300, ErrorMessage = "El campo Comentario solo puede tener {0} caracteres")]
        public string Comment { get; set; }
        public string Creadopor { get; set; }
        public DateTime Fechacreacion { get; set; }
        public string Modificadopor { get; set; }
        public DateTime? Fechamodificacion { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(3, ErrorMessage = "El campo Clave Prueba solo puede tener {0} caracteres")]
        public string KeyTest { get; set; }


        [DataType(DataType.Text)]
        [MaxLength(10, ErrorMessage = "El campo Laboratorio solo puede tener {0} caracteres")]
        public string Laboratory { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(3, ErrorMessage = "El campo Norma solo puede tener {0} caracteres")]
        public string Rule { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(10, ErrorMessage = "El campo Alimentación solo puede tener 10 caracteres")]
        public string Feeding { get; set; }


        [Range(0.001, 999.999, ErrorMessage = "El campo Valor de Alimentación debe ser numérico mayor a cero considerando hasta 3 enteros y 3 decimales")]
        public decimal FeedValue { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(15, ErrorMessage = "El campo Unidad de medida de Alimentación solo puede tener {0} caracteres")]
        public string UmFeeding { get; set; }

        [DataType(DataType.Text)]
        [MaxLength(200, ErrorMessage = "El campo Clave Prueba solo puede tener {0} caracteres")]
        public string CoolingType { get; set; }




        [Range(0, 99, ErrorMessage = "El campo Cantidad de mediciones debe ser numérico mayor a cero considerando 2 enteros sin decimales")]
        public decimal QtyMeasurements { get; set; }
        public bool LoadInfo { get; set; }
        public DateTime DateData { get; set; }

        [Range(0, 99, ErrorMessage = "El campo Cantidad de mediciones Antes y Después debe ser numérico mayor a cero considerando 2 enteros sin decimales")]
        public int MediAyd { get; set; }
        public DateTime TestDate { get; set; }


   


        [DataType(DataType.Text)]
        [MaxLength(150, ErrorMessage = "El campo PosTER solo puede tener {0} caracteres")]
        public string Grades { get; set; }





        public NRATestsDto Data { get; set; }
    }
}
